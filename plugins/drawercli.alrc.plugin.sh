...
EOF
if [[ ! -x $(type -p drawercli) ]];
  then
   
  if timeout 10s curl -fSsl "https://raw.githubusercontent.com/luisadha/drawercli/main/drawercli.sh" -o ~/.local/bin/drawercli && chmod +x ~/.local/bin/drawercli; 
  then
 i_install
  else
   set +e
...
set +e
i_linkin
    ln -s ~/.local/bin/drawercli ~/.shortcuts/drawercli
